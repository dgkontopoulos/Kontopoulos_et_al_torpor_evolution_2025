...
# 601 birds, accounting for phylogeny. 3 of these variables 
...
#
# Rscript fit_MCMCglmm_with_body_mass_corrections_only_across_birds.R 1
# Rscript fit_MCMCglmm_with_body_mass_corrections_only_across_birds.R 2
# Rscript fit_MCMCglmm_with_body_mass_corrections_only_across_birds.R 3
# Rscript fit_MCMCglmm_with_body_mass_corrections_only_across_birds.R 4
# Rscript fit_MCMCglmm_with_body_mass_corrections_only_across_birds.R 5
#
...
# ../Results/MCMCglmm_fits_birds_with_mass_corrections/ directory.
#
...
prepare_vars_for_MCMCglmm <- function()
{
	...
	# For the last common ancestor of Aves, we have information on brain 
	# mass but not body mass. Therefore, for that species, the mass 
	# correction cannot be readily performed, and the data would have to 
	# be dropped.
	#
	# To avoid this, for the last common ancestor of Aves, we extracted 
	# the median posterior estimate of body mass from the mass-uncorrected 
	# MCMCglmm fits (obtained with the fit_MCMCglmm_without_body_mass_corrections_only_across_birds.R 
	# script). We then treated that estimate as 'data', allowing us to 
	# apply the mass correction and to not discard any real data.
	dataset$Body_mass_g[!is.na(dataset$Species) & dataset$Species == 'Aves'] <- 1918.669
	...
	dataset$BMR_Watt_mass_corrected <- dataset$BMR_Watt / dataset$Body_mass_g^0.616
	dataset$Brain_size_g_mass_corrected <- dataset$Brain_size_g / dataset$Body_mass_g^0.556
	dataset$Max_longevity_years_mass_corrected <- dataset$Max_longevity_years / dataset$Body_mass_g^0.190
...
# file.
Sys.sleep(10)
dir.create('../Results/MCMCglmm_fits_birds_with_mass_corrections/', showWarnings = FALSE)
save(fit, file = paste('../Results/MCMCglmm_fits_birds_with_mass_corrections/', chain_id, '.Rda', sep = ''))
