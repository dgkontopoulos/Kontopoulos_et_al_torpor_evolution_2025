...
# ../Results/MCMCglmm_fits_with_mass_corrections_N_random_shifts_seed_N/ directory.
#
...
prepare_vars_for_MCMCglmm <- function(chain_id, random_shifts, user_seed)
{
...
# Read the chain ID, the number of random shifts, and the seed provided 
# by the user as command line arguments.
args <- commandArgs(TRUE)
chain_id <- as.numeric(args[1])
random_shifts <- as.numeric(args[2])
user_seed <- as.numeric(args[3])

# Prepare all the variables needed for fitting the model.
vars_for_MCMCglmm <- prepare_vars_for_MCMCglmm(
	chain_id, random_shifts, user_seed
)
...
dir.create(
	paste(
		'../Results/MCMCglmm_fits_with_mass_corrections_', 
		random_shifts, '_random_shifts_seed_', user_seed, '/', sep = ''
	), 
	showWarnings = FALSE
)
save(
	fit, file = paste(
		'../Results/MCMCglmm_fits_with_mass_corrections_', 
		random_shifts, '_random_shifts_seed_', user_seed, '/', chain_id, 
		'.Rda', sep = ''
	)
)
