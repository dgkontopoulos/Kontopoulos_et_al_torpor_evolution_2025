...
# providing the number of torpor-incapable mammals that are randomly 
# shifted to torpor, as well as the random seed, e.g.:
#
# Rscript fit_Mk_variants_with_some_torpor_incapable_mammals_shifted_to_torpor.R 20 1
...
fit_models <- function(tree, torpor_vector, random_shifts, user_seed)
{
	...
	placental_mammals_without_torpor <- which(
		names(torpor_vector) %in% placental_mammals &
		torpor_vector == 'NO'
	)
	...
	set.seed(1)
	fit_Mk_ER <- fitMk(tree, torpor_vector, model = 'ER', pi='fitzjohn', lik.func="pruning")
	AIC_fit_Mk_ER <- AIC(fit_Mk_ER)
	
	# Forward and backward transition rates are equal.
	cat('Now fitting the Mk_SYM variant...\n')
	set.seed(2)
	fit_Mk_SYM <- fitMk(tree, torpor_vector, model = 'SYM', pi='fitzjohn', lik.func="pruning")
	AIC_fit_Mk_SYM <- AIC(fit_Mk_SYM)
	
	# All transition rates can differ from each other.
	cat('Now fitting the Mk_ARD variant...\n')
	set.seed(3)
	fit_Mk_ARD <- fitMk(tree, torpor_vector, model = 'ARD', pi='fitzjohn', lik.func="pruning")
	...
	set.seed(4)
	lambda_Mk_ER <- determine_optimum_lambda(tree, torpor_vector, model = 'ER')
	fit_Mk_ER_lambda <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_ER), torpor_vector, model = 'ER', pi='fitzjohn', lik.func="pruning")
	...
	set.seed(5)
	lambda_Mk_SYM <- determine_optimum_lambda(tree, torpor_vector, model = 'SYM')
	fit_Mk_SYM_lambda <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_SYM), torpor_vector, model = 'SYM', pi='fitzjohn', lik.func="pruning")
	...
	set.seed(6)
	lambda_Mk_ARD <- determine_optimum_lambda(tree, torpor_vector, model = 'ARD')
	fit_Mk_ARD_lambda <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_ARD), torpor_vector, model = 'ARD', pi='fitzjohn', lik.func="pruning")
	...
	fit_Mk_ER_ordered <- fitMk(tree, torpor_vector, model = matrix_ER, pi='fitzjohn', lik.func="pruning")
	...
	matrix_SYM <-matrix(c(0,0,1,0,0,2,1,2,0),3,3,byrow=TRUE,
	    dimnames=list(c("Hibernation","NO","Torpor"),c("Hibernation","NO","Torpor")))
	fit_Mk_SYM_ordered <- fitMk(tree, torpor_vector, model = matrix_SYM, pi='fitzjohn', lik.func="pruning")
	...
	matrix_ARD <-matrix(c(0,0,1,0,0,2,3,4,0),3,3,byrow=TRUE,
	    dimnames=list(c("Hibernation","NO","Torpor"),c("Hibernation","NO","Torpor")))
	fit_Mk_ARD_ordered <- fitMk(tree, torpor_vector, model = matrix_ARD, pi='fitzjohn', lik.func="pruning")
	...
	set.seed(10)
	lambda_Mk_ER_ordered <- determine_optimum_lambda(tree, torpor_vector, model = matrix_ER)
	fit_Mk_ER_lambda_ordered <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_ER_ordered), torpor_vector, model = matrix_ER, pi='fitzjohn', lik.func="pruning")
	...
	set.seed(11)
	lambda_Mk_SYM_ordered <- determine_optimum_lambda(tree, torpor_vector, model = matrix_SYM)
	fit_Mk_SYM_lambda_ordered <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_SYM_ordered), torpor_vector, model = matrix_SYM, pi='fitzjohn', lik.func="pruning")
	...
	set.seed(12)
	lambda_Mk_ARD_ordered <- determine_optimum_lambda(tree, torpor_vector, model = matrix_ARD)
	fit_Mk_ARD_lambda_ordered <- fitMk(phytools:::lambdaTree(tree, lambda_Mk_ARD_ordered), torpor_vector, model = matrix_ARD, pi='fitzjohn', lik.func="pruning")
...
reconstruct_ancestral_states <- function(tree, random_shifts, user_seed)
{
	...
	simulated_trees <- make.simmap(
		tree, x = results$torpor_vector, Q = results$averaged_Q_matrix,
	...
	write.csv(
		model_estimates,
		file = paste(
			'../Results/torpor_probabilities_Mk_', random_shifts, 
...
fit_models(tree, torpor, random_shifts, user_seed)
